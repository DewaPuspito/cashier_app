...
export class ProductRouter {
  ...
  private routes(): void {
    ...
    this.router.post("/products", AuthenticationMiddleware.verifyToken, AuthorizationMiddleware.allowRoles('ADMIN'), upload.single('imageUrl'), 
    ValidationMiddleware.validate({body: productSchema.body}), this.productController.create.bind(this.productController));
    this.router.put("/products/:id", AuthenticationMiddleware.verifyToken, AuthorizationMiddleware.allowRoles('ADMIN'), upload.single('imageUrl'), 
